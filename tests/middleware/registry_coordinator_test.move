...
module middleware::registry_coordinator_tests {
  ...
  public fun test_init_quorum(deployer: &signer, middleware: &signer, staker: &signer) {
    ...
    registry_coordinator::create_quorum(operator_set_params, minimum_stake, vector::singleton(strategy_params));

    let strategies = service_manager::get_restakeable_strategies();

    assert!(vector::length(&strategies) == 1, 1);
    assert!(fungible_asset::name(*vector::borrow(&strategies, 0)) == string::utf8(b"Token 1"), 2);
  }
}